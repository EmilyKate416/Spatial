...
files <- list.files(split_dir, pattern = "^final_adata_vis_clean_OV_\\d+\\.h5ad$", full.names = TRUE)

# extract sample names
sample_names <- sub("\\.h5ad$", "", basename(files))  # "final_adata_vis_clean_OV_1" etc.

seurat_list <- lapply(seq_along(files), function(i) {
  f <- files[i]
  adata <- zellkonverter::readH5AD(f)

  # 1) Convert to Seurat with your assays
  seurat_obj <- as.Seurat(
    adata,
    counts = "raw_counts",
    data   = "X"
  ...
  # 3) Coordinates and image info
  coords <- reducedDim(adata, "spatial")
  coords_list <- split(as.data.frame(coords), sample)
  image_list  <- metadata(adata)$spatial

  # 4) Assign images/coords as simple lists (your original approach)
  for (sample_name in names(coords_list)) {
    sp_coords <- coords_list[[sample_name]]
    img_info  <- image_list[[sample_name]]
  ...
  seurat_obj@misc$spatial_coords <- coords_list
  seurat_obj@misc$spatial_images <- image_list
  seurat_obj@misc$from_h5ad_file <- basename(f)

  seurat_obj
})

names(seurat_list) <- sub("^.*_(\\d+)\\.h5ad$", "OV_\\1", basename(files))
